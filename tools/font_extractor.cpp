...
static void processFontFile(const std::filesystem::path& otfPath,
    ...
    // Write the binary .font file alongside the SVG directory.
    // The .font file sits next to the .otf (same directory, stem + ".font").
    std::filesystem::path fontBinaryPath = otfPath.parent_path() / (otfPath.stem().string() + ".font");

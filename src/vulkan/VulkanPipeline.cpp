...
void VulkanPipeline::cleanup() {
    ...
    if (m_vectorPipeline != VK_NULL_HANDLE) {
        vkDestroyPipeline(m_device, m_vectorPipeline, nullptr);
        m_vectorPipeline = VK_NULL_HANDLE;
    }
...
void VulkanPipeline::createWaterPipeline(Uint64 id, const ResourceData& vertShader, const ResourceData& fragShader, Uint32 numTextures) {
    ...
    // Use the standard textured vertex binding (dual texture format)
    VkVertexInputBindingDescription bindingDescription{};
    bindingDescription.binding = 0;
    bindingDescription.stride = sizeof(float) * 10;
    bindingDescription.inputRate = VK_VERTEX_INPUT_RATE_VERTEX;
    ...
    // Use water pipeline layout (includes dual texture + light + water polygon)
    VkPipelineLayout pipelineLayout = m_descriptorManager->getWaterPipelineLayout();
    VkDescriptorSetLayout descriptorSetLayout = m_descriptorManager->getDualTextureLayout();
    ...
    VkResult result = vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipelineCreateInfo, nullptr, &pipeline);
    if (result != VK_SUCCESS) {
        m_consoleBuffer->log(SDL_LOG_PRIORITY_ERROR, "vkCreateGraphicsPipelines (water) failed: %s", vkResultToString(result));
        assert(false);
    }
    ...
    void* infoMem = m_allocator->allocate(sizeof(PipelineInfo), "VulkanPipeline::createWaterPipeline::PipelineInfo");
    PipelineInfo* info = new (infoMem) PipelineInfo(*m_allocator);
    info->layout = pipelineLayout;
    info->descriptorSetLayout = descriptorSetLayout;
    info->usesDualTexture = true;  // Water always uses dual textures
    info->usesExtendedPushConstants = true;
    info->usesAnimationPushConstants = true;
    info->isParticlePipeline = false;
    info->isWaterPipeline = true;  // Mark as water pipeline
    m_pipelineInfo.insertNew(id, info);
...
void VulkanPipeline::createParticlePipeline(Uint64 id, const ResourceData& vertShader, const ResourceData& fragShader, int blendMode) {
    ...
    VkVertexInputBindingDescription bindingDescription{};
    bindingDescription.binding = 0;
    bindingDescription.stride = sizeof(float) * 18;
    ...
    // Particle params (halfSize, rotZ, lifeRatio, unused)
    attributeDescriptions[1].binding = 0;
    attributeDescriptions[1].location = 1;
    attributeDescriptions[1].format = VK_FORMAT_R32G32B32A32_SFLOAT;
    attributeDescriptions[1].offset = sizeof(float) * 2;
    ...
    attributeDescriptions[3].location = 3;
    attributeDescriptions[3].format = VK_FORMAT_R32G32B32A32_SFLOAT;
    attributeDescriptions[3].offset = sizeof(float) * 10;

    // UV bounds (u0, v0, u1, v1)
    attributeDescriptions[4].binding = 0;
    attributeDescriptions[4].location = 4;
    attributeDescriptions[4].format = VK_FORMAT_R32G32B32A32_SFLOAT;
    attributeDescriptions[4].offset = sizeof(float) * 14;
...
void VulkanPipeline::destroyPipeline(Uint64 id) {
    ...
    if (pipelinePtr != nullptr) {
        ...
        m_consoleBuffer->log(SDL_LOG_PRIORITY_VERBOSE, "Destroyed pipeline %llu", (unsigned long long)id);
    }
    ...
    if (infoPtrPtr != nullptr) {
        ...
        m_consoleBuffer->log(SDL_LOG_PRIORITY_VERBOSE, "Freed PipelineInfo for pipeline %llu", (unsigned long long)id);
    }
    ...
    if (paramsVecPtr != nullptr) {
        ...
        m_consoleBuffer->log(SDL_LOG_PRIORITY_VERBOSE, "Freed shader parameters for pipeline %llu", (unsigned long long)id);
    }
    ...
    if (ripplesVecPtr != nullptr) {
        ...
        m_consoleBuffer->log(SDL_LOG_PRIORITY_VERBOSE, "Freed water ripples for pipeline %llu", (unsigned long long)id);
    }
...
void VulkanPipeline::createVectorPipeline(const ResourceData& vertShader, const ResourceData& fragShader) {
    if (m_vectorPipeline != VK_NULL_HANDLE) {
        // Already created; skip
        return;
    }

    Vector<char> vertData(*m_allocator, "VulkanPipeline::createVectorPipeline::vertData");
    Vector<char> fragData(*m_allocator, "VulkanPipeline::createVectorPipeline::fragData");
    for (Uint64 i = 0; i < vertShader.size; ++i) {
        vertData.push_back(vertShader.data[i]);
    }
    for (Uint64 i = 0; i < fragShader.size; ++i) {
    ...
    if (result != VK_SUCCESS) {
        m_consoleBuffer->log(SDL_LOG_PRIORITY_ERROR, "vkCreatePipelineLayout (vector) failed: %s", vkResultToString(result));
        assert(false);
    }
    ...
    pipelineInfo.pMultisampleState = &multisampling;
    pipelineInfo.pColorBlendState = &colorBlending;
    pipelineInfo.layout = m_vectorPipelineLayout;
    pipelineInfo.renderPass = m_renderPass;
    pipelineInfo.subpass = 0;
    pipelineInfo.basePipelineHandle = VK_NULL_HANDLE;

    result = vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_vectorPipeline);
    if (result != VK_SUCCESS) {
        m_consoleBuffer->log(SDL_LOG_PRIORITY_ERROR, "vkCreateGraphicsPipelines (vector) failed: %s", vkResultToString(result));
        assert(false);
    }
